...
#реализация отправки emal сообщений
...
def post_detail(request, year, month, day, post):
    post = get_object_or_404(Post,
                             ...
                             publish__day=day)
    #список активных комментариев к данному посту 
    comments = post.comments.filter(active = True)

    #форма для комментариев пользователей
    form = CommentForm()

    #Cписок похожих постов
...
def post_comment(request, post_id):
    ...
    if form.is_valid():
        #создать объект commend не сохраняя его в базе данных
